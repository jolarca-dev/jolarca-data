...
the incident process in `jol-m-infrastructure` and SECURITY.md.
...
1. **Quarantine.** Pause downstream refresh of the affected marts;
   ...
   immediately).
2. **Notify the DPO** (`jol-m-compliance`) — before further technical
...
6. **Evidence.** Record timeline, scope counts, actions, and the DPO's
   notification decision. Evidence custody: `jol-m-compliance`.
